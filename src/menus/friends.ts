...
import { ServerData, drawMainMenu, friends, getData, webScraper } from "..";
...
export async function addFriend(name = "") {
  ...
  if (resp.selectedText == `Name: ${name}`) {
    ...
    if (!name) {
      ...
      terminal.red(" You need to provide a name...");
    ...
    if (friends.friends.includes(name)) {
      ...
      terminal.red(` ${name} is already added as a friend...`);
    ...
    terminal.green(` Added ${name} as a friend.`);
...
export async function drawSelectedUser(
  ...
  switch (resp.selectedText) {
    ...
    case "Remove Friend": {
      ...
      let sp = await terminal.spinner("impulse");
      terminal.green(`  Removed ${name} as a friend.`);
...
async function drawPlayerStats(name: string, stats?: PlayerStats) {
  ...
  terminal(`  Fetching stats for ${name}...`);
